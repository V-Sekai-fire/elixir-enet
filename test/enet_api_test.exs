defmodule Enet.ApiTest do
  ...
  test "local worker init error" do
    ...
    {:ok, local_host} = Enet.start_host(0, connect_fun, peer_limit: 1)
    {:ok, remote_host} = Enet.start_host(0, connect_fun, peer_limit: 1)
    {:ok, local_peer} = Enet.connect_peer(local_host, "127.0.0.1", remote_host, 1)

    ref = Process.monitor(local_peer)

    assert_receive {:DOWN, ^ref, :process, ^local_peer, {:worker_init_error, :whatever}}, 1000

    :ok = Enet.stop_host(local_host)
    :ok = Enet.stop_host(remote_host)
  end

  test "local zero peer limit" do
    self_pid = self()
    connect_fun = fn _peer_info -> {:ok, self_pid} end
    {:ok, local_host} = Enet.start_host(0, connect_fun, peer_limit: 0)
    {:ok, remote_host} = Enet.start_host(0, connect_fun, peer_limit: 1)

    assert {:error, :reached_peer_limit} =
             Enet.connect_peer(local_host, "127.0.0.1", remote_host, 1)
  ...
  test "remote zero peer limit" do
    ...
    {:ok, local_host} = Enet.start_host(0, connect_fun, peer_limit: 1)
    {:ok, remote_host} = Enet.start_host(0, connect_fun, peer_limit: 0)
    {:ok, _peer} = Enet.connect_peer(local_host, "127.0.0.1", remote_host, 1)
  ...
  test "broadcast connect" do
    ...
    {:ok, local_host} = Enet.start_host(0, connect_fun, peer_limit: 1)
    {:ok, remote_host} = Enet.start_host(0, connect_fun, peer_limit: 1)
    {:ok, local_peer} = Enet.connect_peer(local_host, "255.255.255.255", remote_host, 1)
    ...
    :ok = Enet.disconnect_peer(local_peer)
  ...
  test "local disconnect" do
    ...
    {:ok, local_host} = Enet.start_host(0, connect_fun, peer_limit: 8)
    {:ok, remote_host} = Enet.start_host(0, connect_fun, peer_limit: 8)
    {:ok, local_peer} = Enet.connect_peer(local_host, "127.0.0.1", remote_host, 1)
  ...
  test "remote disconnect" do
    ...
    :ok = Enet.disconnect_peer(remote_peer)
  ...
  test "unsequenced messages" do
    ...
    :ok = Enet.send_unsequenced(local_channel1, <<"local->remote">>)
    :ok = Enet.send_unsequenced(remote_channel1, <<"remote->local">>)
  ...
  test "unreliable messages" do
    ...
    :ok = Enet.send_unreliable(local_channel1, <<"local->remote 1">>)
    :ok = Enet.send_unreliable(remote_channel1, <<"remote->local 1">>)
    :ok = Enet.send_unreliable(local_channel1, <<"local->remote 2">>)
    :ok = Enet.send_unreliable(remote_channel1, <<"remote->local 2">>)
  ...
  test "reliable messages" do
    ...
    :ok = Enet.send_reliable(local_channel1, <<"local->remote 1">>)
    :ok = Enet.send_reliable(remote_channel1, <<"remote->local 1">>)
    :ok = Enet.send_reliable(local_channel1, <<"local->remote 2">>)
    :ok = Enet.send_reliable(remote_channel1, <<"remote->local 2">>)
  ...
  test "unsequenced broadcast" do
    ...
    {:ok, host1} = Enet.start_host(0, connect_fun, peer_limit: 8)
    {:ok, host2} = Enet.start_host(0, connect_fun, peer_limit: 8)
    {:ok, host3} = Enet.start_host(0, connect_fun, peer_limit: 8)
